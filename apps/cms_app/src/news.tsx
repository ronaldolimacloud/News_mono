...
// Render with authentication wrapper
ReactDOM.createRoot(document.getElementById("root")!).render(
  <Authenticator>
    {({ signOut, user }) => <NewsApp signOut={signOut as () => void} user={user} />}
  </Authenticator>
);

// Update component to accept auth props
function NewsApp({ signOut, user }: { signOut: () => void, user: any }) {
  ...
  function handleNewsFormChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) {
    ...
    setNewsForm((prev: typeof initialNewsForm) => ({ ...prev, [name]: value }));
  ...
  async function insertImageIntoBody() {
    ...
    try {
      ...
      setNewsForm((prev: typeof initialNewsForm) => ({ ...prev, body: newText }));
    ...
    <main className="w-full">
      <header className="bg-white border-b border-gray-200 py-8">
        <div className="flex justify-between items-center max-w-5xl mx-auto px-4">
          <h1 className="text-4xl font-bold text-gray-800">News Portal</h1>
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-600">Hello {user?.username}</span>
            <button onClick={signOut} className="px-4 py-2 bg-red-100 text-red-700 rounded hover:bg-red-200">Sign out</button>
          </div>
        </div>
